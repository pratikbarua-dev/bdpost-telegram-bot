...
- **AliExpress / Cainiao Tracking Chain Discovery**:
  - Automatically discovers and links multi-stage tracking numbers (`AP...` → `CNG...` → `UG...` / local post) into a single unified shipment.
  - Users only enter their original AliExpress order number (e.g. `AP00839881455575`), and the bot automatically resolves subsequent identifiers.
  - Presents one continuous shipment without creating duplicate parcels.
- **Automated Handover to Bangladesh Post**:
  - Automatically monitors both international Cainiao milestones and local Bangladesh Post events.
  - Switches to Bangladesh Post once the parcel enters Bangladesh, stopping international polling and providing continuous local updates.
...
- **Smart Event Deduplication**: Deterministic SHA-256 event hashing prevents duplicate notifications across both providers and linked numbers.
